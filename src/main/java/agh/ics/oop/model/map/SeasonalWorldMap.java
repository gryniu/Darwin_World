...
public class SeasonalWorldMap extends RealWorldMap {
    ...
    private void checkIfHeated(List<Animal> animals, boolean[] isHeated) {
        List<Integer> indexes = IntStream.range(0, animals.size())
                .boxed()
                .sorted(Comparator.comparingInt(i -> animals.get(i).getPosition().x()))
                .toList();
        ...
        for (int idx : indexes) {
            ...
            activeSet.removeIf(i -> current.getPosition().x() - animals.get(i).getPosition().x() > seasonsOptions.distanceRequiredToHeat());

            for (int i : activeSet) {
                Animal other = animals.get(i);
                if (Vector2d.getDistance(current.getPosition(), other.getPosition()) <= seasonsOptions.distanceRequiredToHeat()) {
                    isHeated[idx] = true;
                    isHeated[i] = true;
                }
            }
